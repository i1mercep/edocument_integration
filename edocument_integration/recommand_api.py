...
class RecommandAPIClient:
	...
	def send_document(self, company_id: str, xml_content: str, document_type: str = "xml") -> Dict[str, Any]:
		...
		try:
			...
			root = ET.fromstring(xml_content_str.encode('utf-8') if isinstance(xml_content_str, str) else xml_content_str)
			
			# Find customer EndpointID using namespace dictionary
			customer_endpoint = root.find('.//cac:AccountingCustomerParty/cac:Party/cbc:EndpointID', UBL_NAMESPACES)
			...
			raise Exception(f"Failed to extract recipient from XML: {str(e)}")
		...
		try:
			...
			if not response.ok:
				try:
					...
					if error_messages:
						error_msg = f"Recommand API error ({response.status_code}): {'; '.join(error_messages)}"
					elif error_message:
	...
	def get_inbox(self, team_id: str, company_id: str = None) -> Dict[str, Any]:
		"""
...
def validate_recommand_connection(api_key: str, api_secret: str, base_url: str = "https://peppol.recommand.eu") -> Dict[str, Any]:
	"""
...
def poll_inbox(integration_settings: Dict[str, Any] = None, company_id: str = None) -> Dict[str, Any]:
	"""
	...
	for doc in documents:
		try:
			...
			frappe.log_error(
				f"Failed to fetch document {doc.get('id')}: {str(e)}\nTraceback: {frappe.get_traceback()}",
				"Recommand Inbox Polling Error"
			)
