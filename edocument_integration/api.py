...
def get_edocument_integration_settings(profile, company=None):
	...
	settings = frappe.get_all(
		...
		filters=filters,
		fields=["api_key", "api_secret", "base_url", "edocument_integrator", "company", "account_id", "company_id"]
	)
...
def transmit_edocument(edocument_name):
	...
	try:
		...
		integration_settings = get_edocument_integration_settings(edocument_doc.edocument_profile, edocument_doc.company)
		if not integration_settings:
			frappe.throw(_("No integration settings found for profile: {0}").format(edocument_doc.edocument_profile))
		...
		if integrator == 'B2B Router':
			...
			transmission_result = client.transmit_invoice(xml_content, invoice_doc=edocument_doc, integration_settings=integration_settings)
		elif integrator == 'Recommand':
			from .recommand_api import transmit_invoice
			transmission_result = transmit_invoice(xml_content, invoice_doc=edocument_doc, integration_settings=integration_settings)
		else:
		...
		parts = [f"E-document Transmission Successful:", f"• Transmission ID: {transmission_id}", f"• Tracking ID: {tracking_id}", f"• Status: {status}"]
		...
		frappe.db.set_value("EDocument", edocument_name, {
		...
		frappe.log_error(f"E-document transmission failed for document {edocument_name}: {str(e)}", "E-document Transmission Error")
...
def webhook(**kwargs):
	...
	try:
		...
		request_log = create_request_log(kwargs, request_description="Incoming Document", service_name="EDocument Integration", request_headers=r.headers)
		...
		frappe.db.rollback()
		frappe.log_error(f"E-Document webhook processing failed: {str(e)}\n{frappe.get_traceback()}", "E-Document Webhook Error")
		frappe.db.commit()
	...
	try:
		...
		for invoice_data in poll_result.get('invoices', []):
			try:
				...
				if not xml_bytes:
					frappe.log_error(
						f"No XML content in invoice data: {invoice_data}",
				...
				comment_parts = [f"Incoming invoice received from Recommand"]
				if document_id:
				...
				frappe.log_error(
					f"Failed to create EDocument for incoming invoice: {str(e)}\nTraceback: {frappe.get_traceback()}",
					"Poll Incoming Invoices Error"
				)
				continue
		...
		frappe.log_error(
			f"Poll incoming invoices failed: {str(e)}\nTraceback: {frappe.get_traceback()}",
			"Poll Incoming Invoices Error"
		)
