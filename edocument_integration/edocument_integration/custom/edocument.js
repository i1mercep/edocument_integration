frappe.ui.form.on("EDocument", {
	refresh: function (frm) {
		...
		if (!frm.is_new() && frm.doc.edocument_profile && frm.doc.status === "Validation Successful") {
				...
				frm.call({
					...
					callback: function(r) {
						if (r.message) {
							...
							frm.add_custom_button(__("Send EDocument"), function() {
								frappe.confirm(
									__("Are you sure you want to transmit this document to Peppol Network?"),
									() => {
										// Show loading indicator
										frappe.show_progress(__("Transmitting..."), 0, 100, __("Sending document to Peppol Network"));
										...
										frappe.call({
											...
											callback: function(r) {
												...
												if (r.message) {
													...
													const transmissionId = result.document_id || result.id || result.invoice_id || "N/A";
													
													frappe.show_alert({
														message: __("E-document transmitted successfully. Transmission ID: {0}", [transmissionId]),
														indicator: 'green'
													});
